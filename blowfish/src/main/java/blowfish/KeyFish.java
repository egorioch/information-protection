...
public class KeyFish {
    public HashMap<Integer, String> set_subkeys() {
        File file = new File("src/main/resources/pi_values.txt");
        return new HashMap<>();
    }


    // таблица замен
    // 4 матрицы подстановки по 256 элементов в каждой
    // выбираются случайным образом в процессе инициализации алгоритма ключом
    String[][] S
            = {
            {
    ...
    public KeyFish() {
        ...
        String plainTextDecr = decrypt(cipherText);
